# Final Verification Script - Memastikan rebrand Kilas Janten selesai dengan baik

$WorkspaceRoot = "c:\KULIAH\MAGANG\Magang di Perhutani\Kilas Janten"

Write-Host "========== FINAL VERIFICATION - Kilas Janten REBRAND ==========" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "4. Checking for new branding 'Kilas Janten'..." -ForegroundColor Yellow
$newBrandingFound = Get-ChildItem -Path $WorkspaceRoot -Recurse -Include "*.html" -File |
    Select-String -Pattern "Kilas Janten|kilasjanten|kilasjanten" -ErrorAction SilentlyContinue |
    Where-Object { $_.Path -notlike "*\.bak.*" } | 
    Measure-Object

if ($newBrandingFound.Count -gt 0) {
    Write-Host "   ✅ Found 'Kilas Janten' branding in $($newBrandingFound.Count) places" -ForegroundColor Green
} else {
    Write-Host "   ⚠️  No 'Kilas Janten' branding found!" -ForegroundColor Yellow
    $issues += @{ Type = "Missing"; File = "All"; Reason = "No Kilas Janten branding found" }
}
...
foreach ($pkg in $pkgFiles) {
    ...
    if ($content -match '"name"\s*:\s*"kilasjanten') {
        $pkgOK++
...
Write-Host ""
Write-Host "Rebrand Kilas Janten SELESAI" -ForegroundColor Green
